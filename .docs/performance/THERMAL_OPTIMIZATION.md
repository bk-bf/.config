...
**Date:** February 27, 2026 | **Updated:** April 2026  
...
| Fix                        | State       | Details                                                  |
| -------------------------- | ----------- | -------------------------------------------------------- |
| Power profile              | ✅ OK        | `balanced` — was temporarily set to `performance` (April 2026), reverted |
...
| intel-undervolt            | ✅ Active    | PL1=15W (long), PL2=25W (short); **voltage offsets zeroed out** (stability) |
...
Config: `/etc/intel-undervolt.conf` (symlink → `~/.config/intel-undervolt/intel-undervolt.conf`, source-controlled)
...
**April 2026 note:** CPU/GPU/Cache voltage offsets were zeroed out (all set to `0mV`) for
stability during heavy dev sessions. The power limits (PL1/PL2) remain active and are the
primary thermal constraint. Undervolting can be restored if stability is confirmed.
...
**Related docs:** [S2IDLE_OPTIMIZATION.md](battery/S2IDLE_OPTIMIZATION.md), [BATTERY_NOTIFICATION.md](swayfx+waybar/BATTERY_NOTIFICATION.md)
