// BarWidget.qml — a fork of Noctalia's core SystemMonitor bar widget.
//
// Verbatim copy of Modules/Bar/Widgets/SystemMonitor.qml, so it renders every
// metric with the same layout, fonts, colours and per-widget settings. Forking
// rather than sitting beside it is deliberate: an adjacent widget duplicated the
// memory readout, and turning memory off in the core widget left a plugin
// separator gap in the bar.
//
// Three changes from the original, marked FORK: below.
//   1. pluginApi / svc wiring
//   2. left click opens this plugin's process breakdown instead of the built-in
//      systemStatsPanel
//   3. the hover tooltip no longer consults the built-in panel's open state
...
Item {
  ...
  // FORK: plugin plumbing — mainInstance is the shared mem-top data source.
  ...
  MouseArea {
    ...
    onClicked: mouse => {
                 if (mouse.button === Qt.LeftButton) {
                   // FORK: open the process breakdown, not the built-in stats panel
                   if (root.pluginApi)
    ...
    onEntered: {
      // FORK: the built-in stats panel is not part of this plugin
      {
