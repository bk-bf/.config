// WidgetSettings.qml — the page shown by the BAR widget settings dialog
// (right-click the pill -> Widget settings).
//
// This is a different contract from Settings.qml, which the Plugins tab uses:
// here the component is handed screen/widgetData/widgetMetadata, and
// saveSettings() must EMIT settingsChanged(obj) rather than write plugin state.
//
// It exists because BarWidgetSettingsDialog resolves its page through
// BarWidgetRegistry.widgetSettingsMap, which only ever contains core widget ids
// -- every "plugin:*" id misses and the dialog comes up EMPTY. Main.qml registers
// this file into that map so the dialog has something to load.
//
// Forked from Modules/Panels/Settings/Bar/WidgetSettings/SystemMonitorSettings.qml
// to match the forked bar widget, with the memory-scanner options appended.
...
ColumnLayout {
  ...
  // FORK: memory-scanner options
  property string valueBinPath: widgetData.binPath !== undefined ? widgetData.binPath : "~/.local/bin/mem-top"
  property int valueIntervalMs: widgetData.intervalMs !== undefined ? widgetData.intervalMs : 10000
  property int valueLimit: widgetData.limit !== undefined ? widgetData.limit : 14
  ...
  function saveSettings() {
    ...
    // FORK: memory-scanner options
    settings.binPath = valueBinPath;
    settings.intervalMs = valueIntervalMs;
    settings.limit = valueLimit;
  ...
  NSpinBox {
    ...
    from: 3
    to: 120
    value: Math.round(valueIntervalMs / 1000)
    suffix: " s"
    onValueChanged: {
      valueIntervalMs = value * 1000;
      saveSettings();
    }
  }
  ...
  NSpinBox {
    Layout.fillWidth: true
    label: "Entries shown"
    description: "How many consumers to list; the rest is summed as \"everything else\"."
    from: 5
    to: 40
    value: valueLimit
    onValueChanged: {
