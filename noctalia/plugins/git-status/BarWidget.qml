...
//
// Pure presentation over Main.qml (via pluginApi.mainInstance). Left click opens
// the panel, middle click re-scans, right click gives a context menu. Uses the
// stock BarPill with the Tabler git-commit glyph — one fixed shape in every
// state, tinted by state (green clean / amber pending / red unpulled).
...
Item {
  ...
  BarPill {
    ...
    icon: root.svc ? root.svc.barIconName : "git-commit"
    text: root.svc ? root.svc.barLabel : "…"
    iconPosition: "left"

    // the count carries clean/pending/behind via stateColor
